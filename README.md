...
- **Tipos de columna en SQL de migración escrito a mano**: todos los ids del schema son `String @id @default(uuid())` en Prisma — ningún modelo usa `@db.Uuid`, así que en Postgres son `TEXT`, no `UUID` nativo. Escribir `id`, `tenant_id` o una FK como `UUID` en `migration.sql` rompe la constraint (`incompatible types: uuid and text`). Si escribís SQL a mano, copiá el tipo de una migración existente (ej. `expense_categories`) en vez de asumirlo.
