    ...
    <div className="flex items-center gap-3 mb-4">
      <div className={[
        'w-7 h-7 rounded-full flex items-center justify-center shrink-0 transition-all duration-200',
          ...
          : 'bg-[var(--color-surface-2)] text-gray-600',
      ].join(' ')}>
        {done ? (
...
export function PaymentModal({ isOpen, onClose, total, onConfirm, onDeferPayment, allowCortesia = false }: Props) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      await onConfirm(payments, customerValue, selectedType);
      resetState();
    } catch {
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Nuevo pedido" size="3xl">
      <div className="flex flex-col lg:flex-row lg:gap-x-6">
        ...
        ══════════════════════════════════════ */}
        <div className="flex-1 space-y-5 lg:border-r lg:border-[var(--border-subtle)] lg:pr-6">
          ...
          <div>
            ...
            {customerConfirmed && customerValue === null ? (
              <div className="flex items-center justify-between px-4 py-3 rounded-2xl bg-[var(--color-surface-2)] border-2 border-[var(--border-subtle)]">
                ...
                <button
                  ...
                  className="text-xs text-gray-400 hover:text-gray-600 transition-colors px-2 py-1 rounded-lg hover:bg-[var(--color-surface-2)]"
                >
                  Cambiar
                </button>
              </div>
                ...
                {!customerConfirmed && (
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-2xl
                      border-2 border-[var(--border-subtle)] text-sm font-medium text-gray-500
                      hover:border-[var(--border-strong)] hover:text-gray-600 hover:bg-[var(--color-surface-2)] transition-all duration-150"
                  >
                ...
                {selectedMethod !== PaymentMethod.CORTESIA && (
                  <button
                    ...
                    className="mt-3 w-full flex items-center justify-center gap-1.5 py-2.5 rounded-xl
                      ...
                      border-2 border-[var(--border-subtle)] hover:border-primary-500/40 transition-all"
                  >
                      ...
                      return (
                        <div key={i} className="flex items-center gap-1.5 pl-3 pr-2 py-1.5 rounded-full bg-[var(--color-surface-2)] border border-[var(--border-subtle)] text-sm">
                          ...
                          <button
                            ...
                            className="w-4 h-4 rounded-full bg-[var(--border-subtle)] hover:bg-red-500/60 flex items-center justify-center text-gray-500 hover:text-white transition-colors ml-0.5"
                            aria-label="Quitar"
                          >
          ...
          <div className="border-t border-[var(--border-subtle)] pt-4 space-y-3">
            ...
            {!canConfirm && (
              <div className="flex flex-wrap gap-1.5 px-1">
                {!step1Done && (
                  <span className="text-[11px] px-2 py-0.5 rounded-full bg-[var(--color-surface-2)] text-gray-600">Selecciona tipo de pedido</span>
                )}
                {!step2Done && (
                  <span className="text-[11px] px-2 py-0.5 rounded-full bg-[var(--color-surface-2)] text-gray-600">Confirma el cliente</span>
                )}
                {!step3Done && (
                  <span className="text-[11px] px-2 py-0.5 rounded-full bg-[var(--color-surface-2)] text-gray-600">Elige método de pago</span>
                )}
              </div>
            )}
            ...
            <button
              ...
              className={[
                ...
                canConfirm
                  ...
                  : 'bg-[var(--color-surface-2)] text-gray-500 cursor-not-allowed',
              ].join(' ')}
            >
