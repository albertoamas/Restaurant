...
export function OrderPanel({ onCharge, onClose }: Props) {
  ...
  return (
    <div className="flex flex-col h-full bg-white/95 backdrop-blur-xl">
      ...
      {onClose && (
        <div className="flex items-center justify-between px-4 pt-4 pb-3 border-b border-gray-100/80 shrink-0">
          <div>
      ...
      <div className="flex items-center justify-between px-4 pt-3 pb-1.5 shrink-0">
        ...
        {items.length > 0 && (
          <button
            onClick={clear}
            className="flex items-center gap-1 px-2 py-1 rounded-lg text-[11px] font-semibold text-red-500 hover:bg-red-50 hover:text-red-600 transition-colors"
          >
        ...
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-gray-300 gap-3 py-10">
            <div className="w-16 h-16 rounded-2xl bg-gray-50 flex items-center justify-center">
              <Icon name="cart" size={32} strokeWidth={1} className="opacity-40" />
            </div>
            <div className="text-center">
              <p className="text-sm font-semibold text-gray-400">Carrito vacío</p>
              <p className="text-xs text-gray-300 mt-0.5">Selecciona productos del catálogo</p>
            </div>
          </div>
        ) : (
      ...
      {/* Notes */}
      <div className="px-4 py-2.5 border-t border-gray-100/80 shrink-0">
        <textarea
          rows={2}
          placeholder="Notas: sin cebolla, mesa 5..."
          value={notes}
          onChange={(e) => setNotes(e.target.value)}
          className="w-full text-sm border border-gray-200 rounded-xl px-3 py-2 resize-none bg-gray-50/80
      ...
      {/* Footer total + cobrar */}
      <div className="px-4 pb-4 pt-3 shrink-0 space-y-3 border-t border-gray-100/80">
