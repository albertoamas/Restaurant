...
- **Column types in hand-written migration SQL**: every id in this schema is a Prisma `String @id @default(uuid())` — no model uses `@db.Uuid`. That maps to a plain Postgres `TEXT` column, not native `UUID`. Writing `id`/`tenant_id`/a foreign key as `UUID` in a manual `migration.sql` fails with `foreign key constraint ... cannot be implemented: incompatible types: uuid and text` — Prisma's DDL runs as one transaction, so a failed migration leaves no partial table behind, but it does record a failed row in `_prisma_migrations` that blocks further migrations until resolved (`npx prisma migrate resolve --rolled-back <name>`). Grep an existing migration (e.g. `expense_categories`) for the column type before free-handing it.
- **`prisma migrate reset` / other destructive commands refuse to run for an AI agent**: Prisma detects the Claude Code invocation and requires `PRISMA_USER_CONSENT_FOR_DANGEROUS_AI_ACTION` set to the user's literal, freshly-given consent message. Never set that variable without asking the user in that exact turn what will be destroyed, confirming it's a development database, and getting an explicit yes — and never run it against production.
...
| `admin` | `GET/POST /admin/tenants`, `GET /admin/tenants/:id/health`, `PATCH /admin/tenants/:id/toggle`, `PATCH /admin/tenants/:id/plan`, `PATCH /admin/tenants/:id/modules`, `GET/PATCH /admin/plans` | `AdminGuard`; no JWT. `/modules` sets per-tenant feature flags (`ordersEnabled`, `cashEnabled`, `rafflesEnabled`, etc.). `POST /tenants` crea tenant + dueño + sucursal inicial en una transacción. `/health` es el semáforo de alta: ver `docs/onboarding-cliente.md`. |
